...
AddEventHandler("playerConnecting",function(name,setMessage, deferrals)
  ...
  if ids ~= nil and #ids > 0 then
    deferrals.update("[CRP] Checando identificação...")
    vRP.getUserIdByIdentifiers(ids, function(user_id)
      ...
      if user_id ~= nil then -- check user validity 
        deferrals.update("[CRP] Checando banimento...")
        vRP.isBanned(user_id, function(banned)
          if not banned then
            deferrals.update("[CRP] Checando os Listados...")
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[user_id] == nil then -- not present on the server, init
                  ...
                  deferrals.update("[CRP] Carregando...")
                  vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    deferrals.update("[CRP] Pegando ultima localização...")
                ...
                print("[CRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejeitado: não está na whitelist (user_id = "..user_id..")")
                deferrals.done("[CRP] Este é seu [ID = "..user_id.."] entrar no discord para fazer o formulário para ser adicionado a whitelist. link(Discord):")
              end
            end)
          else
            print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: banned (user_id = "..user_id..")")
            deferrals.done("[CRP] Banido [ID = "..user_id.."].")
          end
        end)
      else
        print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: identification error")
        deferrals.done("[CRP] Erro de indentificação.")
      end
    end)
  else
    print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: missing identifiers")
    deferrals.done("[CRP] Faltando indentificação.")
  end
  Debug.pend()
end)
