...
police_weapons["Rota"] = {function(player,choice)
    vRPclient.giveWeapons(player,{{
	  ["WEAPON_COMBATPISTOL"] = {ammo=60},
	  ["WEAPON_PUMPSHOTGUN"] = {ammo=32},
	  ["WEAPON_NIGHTSTICK"] = {ammo=200},
    ["WEAPON_SNIPERRIFLE"] = {ammo=60},
    ["WEAPON_CARBINERIFLE"] = {ammo=120},
	  ["WEAPON_STUNGUN"] = {ammo=200}
	}, true})
end}

police_weapons["Rota Recruta"] = {function(player,choice)
    vRPclient.giveWeapons(player,{{
	  ["WEAPON_COMBATPISTOL"] = {ammo=60},
	  ["WEAPON_PUMPSHOTGUN"] = {ammo=32},
	  ["WEAPON_NIGHTSTICK"] = {ammo=200},
			...
			if custom ~= nil then
			  ...
			  if tonumber(custom) > 0 or vRP.hasPermission({user_id,"admin.easy_unjail"}) then
				...
				if target ~= nil then
				  ...
				  vRPbm.logInfoToFile("jailLog.txt",user_id .. " libertado "..target_id.." de " .. custom .. " minutos de senteça")
				else
				  vRPclient.notify(player,{"~r~Essa ID parece inválida."})
				end
			  else
	  ...
	  if value ~= nil then
	    ...
	    if custom ~= nil then
		  if tonumber(custom) > 0 then
            ...
            vRPclient.notify(target,{"~r~Acabou sua sentença."})
			vRP.setHunger({tonumber(user_id),0})
			vRP.setThirst({tonumber(user_id),0})
			vRPbm.logInfoToFile("jailLog.txt",user_id.." foi enviado de volta à prisão por " .. custom .. " minutos para completar sua sentença")
...
local ch_player_menu = {function(player,choice)
    ...
    if vRP.hasPermission({user_id,"player.check"}) then
      menu["Inspencionar"] = choice_player_check -- checks nearest player inventory, like police check from vrp
    end
...
vRP.registerMenuBuilder({"main", function(add, data)
  ...
  if user_id ~= nil then
    ...
    if vRP.hasPermission({user_id,"player.player_menu"}) then
      choices["🤴Jogador"] = ch_player_menu -- opens player submenu
    end
    ...
    if vRP.hasPermission({user_id,"player.loot"}) then
      choices["💰Roubar"] = choice_loot -- take the items of nearest player in coma
    end
...
vRP.registerMenuBuilder({"police", function(add, data)
  ...
  if user_id ~= nil then
	...
	if vRP.hasPermission({user_id,"police.easy_unjail"}) then
      choices["Retirar da prisão"] = ch_unjail -- Un jails chosen player if he is jailed (Use admin.easy_unjail as permission to have this in admin menu working in non jailed players)
    end
	...
	if vRP.hasPermission({user_id,"police.easy_cuff"}) then
      choices["Algemar Fácil"] = ch_handcuff -- Toggle cuffs AND CLOSE MENU for nearby player
    end
	
	if vRP.hasPermission({user_id,"police.spikes"}) then
      choices["Espinhos"] = ch_spikes -- Toggle spikes
    end
